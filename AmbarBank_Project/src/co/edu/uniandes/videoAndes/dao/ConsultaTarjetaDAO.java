 ...
 */
public class ConsultaTarjetaDAO {
	...
	/**
	 * nombre de la tabla Tarjetas
	 */
	private static final String tablaTarjeta = "Tarjetas";
	
	
	/**
	 * nombre de la columna titulo_original en la tabla Tarjetas.
	 */
	private static final String tipoTarjeta = "tipo_Tarjeta";
	
	/**
	 * nombre de la columna anyo en la tabla Tarjetas.
	 */
	...
	/**
	 * Consulta que devuelve isan, titulo, y ao de los Tarjetas en orden alfabetico
	 */
	private static final String consultaTarjetasDefault="SELECT *, FROM "+tablaTarjeta;
     ...
     * y retorna un ArrayList de elementos tipo TarjetasValue.
     * @return ArrayList lista que contiene elementos tipo TarjetasValue.
     * La lista contiene los Tarjetas ordenados alfabeticamente
    ...
    public ArrayList<TarjetaValue> darTarjetasDefault() throws Exception
    {
    	...
    	ArrayList<TarjetaValue> Tarjetas = new ArrayList<TarjetaValue>();
		TarjetaValue vidValue = new TarjetaValue();
    	
		try {
			establecerConexion(cadenaConexion, usuario, clave);
			prepStmt = conexion.prepareStatement(consultaTarjetasDefault);
			...
			while(rs.next()){
				...
				vidValue.setNombreBanco(tipo);
				vidValue.setNumeroTarjeta(idOperacion);
				
				Tarjetas.add(vidValue);
				vidValue = new TarjetaValue();
							
			}
		
		} catch (SQLException e) {
			e.printStackTrace();
			System.out.println(consultaTarjetasDefault);
			throw new Exception("ERROR = ConsultaTarjetaDAO: loadRowsBy(..) Agregando parametros y executando el statement!!!");
		}finally 
		{
			if (prepStmt != null) 
			{
				try {
					...
					throw new Exception("ERROR: ConsultaTarjetaDAO: loadRow() =  cerrando una conexion.");
				}
			}
