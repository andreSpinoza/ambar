 ...
 */
public class ConsultaTransferenciaDAO {
	...
	/**
	 * nombre de la tabla Transferencias
	 */
	private static final String tablaTransferencia = "Transferencias";
	
	
	/**
	 * nombre de la columna titulo_original en la tabla Transferencias.
	 */
	private static final String tipoTransferencia = "tipo_Transferencia";
	
	/**
	 * nombre de la columna anyo en la tabla Transferencias.
	 */
	...
	/**
	 * Consulta que devuelve isan, titulo, y ao de los Transferencias en orden alfabetico
	 */
	private static final String consultaTransferenciasDefault="SELECT *, FROM "+tablaTransferencia;
	 ...
	 */
	public ConsultaTransferenciaDAO() 
	{		
     ...
     * y retorna un ArrayList de elementos tipo TransferenciasValue.
     * @return ArrayList lista que contiene elementos tipo TransferenciasValue.
     * La lista contiene los Transferencias ordenados alfabeticamente
    ...
    public ArrayList<TransferenciaValue> darTransferenciasDefault() throws Exception
    {
    	...
    	ArrayList<TransferenciaValue> Transferencias = new ArrayList<TransferenciaValue>();
		TransferenciaValue vidValue = new TransferenciaValue();
    	
		try {
			establecerConexion(cadenaConexion, usuario, clave);
			prepStmt = conexion.prepareStatement(consultaTransferenciasDefault);
			
			ResultSet rs = prepStmt.executeQuery();
			
			while(rs.next()){
				String tipo= rs.getString(tipoTransferencia);
				int idOperacion = rs.getInt(idOp);
				
				vidValue.setEstado(tipo);
				vidValue.setCantidad(idOperacion);
				
				Transferencias.add(vidValue);
				vidValue = new TransferenciaValue();
							
			}
		
		} catch (SQLException e) {
			e.printStackTrace();
			System.out.println(consultaTransferenciasDefault);
			throw new Exception("ERROR = ConsultaTransferenciaDAO: loadRowsBy(..) Agregando parametros y executando el statement!!!");
		}finally 
		{
			if (prepStmt != null) 
			{
				try {
					...
					throw new Exception("ERROR: ConsultaTransferenciaDAO: loadRow() =  cerrando una conexion.");
				}
			}
