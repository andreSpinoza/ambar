...
public class ConsultaClienteDAO {
	...
	/**
	 * nombre de la columna titulo_original en la tabla Clientes.
	 */
	private static final String tipoCliente = "tipo_Cliente";
	
	/**
	 * nombre de la columna anyo en la tabla Clientes.
	 */
	...
	/**
	 * Consulta que devuelve isan, titulo, y ao de los Clientes en orden alfabetico
	 */
     ...
     * y retorna un ArrayList de elementos tipo ClientesValue.
     * @return ArrayList lista que contiene elementos tipo ClientesValue.
     * La lista contiene los Clientes ordenados alfabeticamente
    ...
    public ArrayList<ClienteValue> darClientesDefault() throws Exception
    {
		...
		ClienteValue vidValue = new ClienteValue();
		...
		try {
			...
			while(rs.next()){
				...
				vidValue.setTipoCliente(tipo);
				vidValue.setIdcliente(idOperacion);
				
				Clientes.add(vidValue);
				vidValue = new ClienteValue();
			...
			throw new Exception("ERROR = ConsultaClienteDAO: loadRowsBy(..) Agregando parametros y executando el statement!!!");
		}finally 
		{
			if (prepStmt != null) 
			{
				try {
					...
					throw new Exception("ERROR: ConsultaClienteDAO: loadRow() =  cerrando una conexion.");
				}
			}
